...
Como Senior Technical Recruiter o Account Recruitment Coordinator de TCS,  
...
1. **Scenario 1.1 (Autollenado exitoso por DNI y registro auditable de creación):**  
   **Given** que la Senior Technical Recruiter se encuentra en el formulario de creación de ficha de candidato con una sesión autenticada activa,  
   **When** ingresa un número de DNI peruano válido de 8 dígitos (ej. `76128709`) y presiona validar o cambia de campo,  
   **Then** el sistema resuelve la consulta prioritariamente a través del almacén/caché local de identidades o consulta en tiempo real el servicio de validación de identidad nacional persistiendo los datos obtenidos en la caché local, autocompleta de inmediato los campos de nombres de pila, apellido paterno, apellido materno, fecha de nacimiento y distrito de residencia, calculando la edad cronológica exacta a la fecha actual, bloqueando la edición de campos de identidad oficial para evitar alteraciones accidentales, y consignando al guardar la autoría del registro a la Senior Technical Recruiter autenticada en la bitácora inmutable de auditoría.

2. **Scenario 1.2 (Normalización canónica de teléfono a E.164 y enlace a WhatsApp):**  
   **Given** que la Senior Technical Recruiter ingresa un teléfono de contacto en cualquiera de sus formatos habituales (ej. `989322088`, `51989322088`, `+51 989 322 088` o `989-322-088`),  
...
3. **Scenario 1.3 (Detección reactiva inmediata de candidato duplicado en entrada):**  
   ...
   **When** una Senior Technical Recruiter intenta crear una nueva ficha utilizando dicho DNI, celular o correo electrónico,  
   **Then** el sistema interrumpe el registro, muestra una alerta no bloqueante indicando el duplicado, despliega el nombre del candidato, el Senior Technical Recruiter responsable actual, la fecha del último contacto y un enlace directo para abrir la ficha histórica existente en lugar de crear un registro huérfano.

4. **Scenario 1.4 (Extracción estructurada inicial de CV adjunto con custodia auditable):**  
   **Given** que la Senior Technical Recruiter autenticada carga un archivo de currículo en formato estándar (PDF o Word) al crear o editar una ficha,  
   **When** se completa la carga documental,  
   **Then** el sistema extrae de forma asistida y sin sesgos demográficos el resumen de competencias tecnológicas principales, años estimados de experiencia laboral, enlaces profesionales externos y nivel de idiomas, presentándolos en campos estructurados editables para confirmación humana antes de guardar, y asocia de forma permanente el archivo almacenado con el usuario autenticado, registrando en auditoría el nombre del documento, tamaño en bytes e identificador de integridad.

5. **Scenario 1.5 (Captura de las 7 dimensiones de la llamada humana con autoría individual y alerta geográfica):**  
   **Given** que la Senior Technical Recruiter autenticada realiza la llamada telefónica de screening al candidato,  
   **When** ingresa a la sección de validación cualitativa en la ficha de postulación y guarda la evaluación,  
   **Then** el sistema provee campos estructurados para registrar los 7 objetivos obligatorios: disponibilidad de inicio (Inmediata, 1 semana, 2 semanas, 1 mes), resumen técnico validado, expectativa salarial, interés en la vacante, aceptación de modalidad (híbrido/remoto), viabilidad de traslado geográfico con alerta de distancia (ej. alerta si reside en Villa María del Triunfo y la sede asignada es BCP La Molina), e impresión general, salvaguardando que la llamada y su veredicto sean ejecutados exclusivamente por una persona humana y vinculando la autoría indeleble del dictamen al usuario autenticado.

6. **Scenario 1.6 (Respuesta ante DNI no encontrado en el padrón oficial o ausencia de red con DNI nuevo):**  
   **Given** que la Senior Technical Recruiter ingresa un número de 8 dígitos que no existe en los registros del padrón nacional de identidad o se opera en ausencia de conectividad de red para un DNI no presente en la caché local,  
   **When** se ejecuta la validación de identidad,  
   **Then** el sistema notifica la indisponibilidad o ausencia de registro, habilita la captura asistida mediante edición manual de nombres, apellidos y fecha de nacimiento, asigna a la ficha el distintivo y marca de auditoría "Pendiente de regularización de identidad" y encola la solicitud para sincronización y validación automática al restablecerse el servicio.

---

### User Story 2 - Validador Masivo de Planillas Excel de Adecco con Semáforo Algorítmico y Detección Cruzada (Priority: P2)

Como Senior Technical Recruiter responsable de la gestión de proveedores externos,  
...
1. **Scenario 2.1 (Clasificación 🔴 Rojo - Duplicado Activo o Descarte Reciente):**  
   ...
   **Then** marca la fila con distintivo rojo (🔴), indica el motivo de la coincidencia ("En proceso activo en cuenta BCP" o "Descartado el 15/07/2026: No superó filtro técnico"), señala al Senior Technical Recruiter responsable y excluye el perfil de la lista limpia a procesar.
...
4. **Scenario 2.4 (Clasificación 🟢 Verde - Candidato Inédito y Limpio con Trazabilidad de Ingesta):**  
   ...
   **When** concluye la evaluación del archivo y la Senior Technical Recruiter autenticada confirma la importación,  
   **Then** marca la fila con distintivo verde (🟢), importa masivamente los perfiles limpios hacia la bandeja de entrada de candidatos nuevos listos para llamada de screening, y asienta en la bitácora inmutable la creación de cada ficha vinculada al lote y al identificador de la usuaria que ejecutó la importación.
...
6. **Scenario 2.6 (Tolerancia a esquemas heterogéneos y encabezados con alias):**  
   ...
   **Then** el sistema mapea semánticamente los encabezados contra el diccionario canónico, omite filas vacías y procesa los datos sin requerir que la Senior Technical Recruiter modifique manualmente el archivo Excel original.

---

### User Story 3 - Generador a Demanda de Reporte de Cartera y Exclusiones para Adecco según Ley 29733 (Priority: P3)

Como Senior Technical Recruiter o Head of Talent Acquisition,  
quiero generar y descargar en cualquier momento con un solo clic el Reporte de Cartera y Exclusiones para Adecco estructurado uniformemente con las 5 columnas oficiales (`DNI`, `Nombres y Apellidos`, `Perfil`, `Vigencia de Exclusión`, `Estado`),  
para entregar al proveedor la lista estandarizada de profesionales que TCS ya tiene en proceso, en cartera o en periodo de enfriamiento, garantizando el estricto cumplimiento de la Ley N° 29733 (Ley de Protección de Datos Personales de Perú) al censurar y omitir datos sensibles como pretensiones salariales, notas privadas, teléfonos personales y evaluaciones internas.

**Why this priority**: Ataca la causa raíz del problema con el proveedor externo. Adecco remite candidatos duplicados porque busca a ciegas. Si TCS le entrega periódicamente a demanda el reporte de cartera y exclusiones antes de que inicien sus búsquedas, se evita que busquen a los mismos candidatos, elevando el ratio de CV útil del 10% actual hacia más del 35% y eliminando el retrabajo de raíz.

**Independent Test**: Puede ser probado de forma independiente pulsando el botón "Generar Reporte de Cartera y Exclusiones Adecco" en la aplicación. El sistema compila los candidatos en estado activo, cartera y exclusión temporal, estructura uniformemente las 5 columnas oficiales (`DNI`, `Nombres y Apellidos`, `Perfil`, `Vigencia de Exclusión`, `Estado`), omitiendo remuneraciones, correo, celular y notas privadas bajo la Ley N° 29733, y descarga un archivo en formato estructurado estándar (.xlsx/.csv).

**Acceptance Scenarios**:

1. **Scenario 3.1 (Generación a demanda en 1-clic):**  
   **Given** que existen 450 candidatos en el sistema clasificados en estados que ameritan exclusión o cartera activa (en proceso de screening, entrevistas con clientes, ofertas en curso o descartados hace menos de 180 días),  
   **When** la Senior Technical Recruiter hace clic en "Descargar Reporte de Cartera y Exclusiones Adecco",  
   **Then** el sistema compila en menos de 2 segundos el archivo descargable actualizado al instante exacto con las 5 columnas oficiales, registrando en la auditoría del sistema la fecha, hora y usuario que efectuó la descarga.

2. **Scenario 3.2 (Estructura oficial de 5 columnas y anonimización estricta bajo Ley N° 29733):**  
   ...
   **Then** el archivo contiene de manera uniforme y exclusiva las 5 columnas oficiales: `DNI`, `Nombres y Apellidos`, `Perfil`, `Vigencia de Exclusión` y `Estado`; quedando estrictamente censurados y excluidos el número telefónico personal, correo electrónico, pretensión salarial, tarifas de facturación del cliente, notas cualitativas de descarte y cualquier atributo sensible protegido bajo la Ley N° 29733.
...
3. **Scenario 3.3 (Filtro por cuenta o cliente específico):**  
   ...
   **When** la Senior Technical Recruiter selecciona la cuenta antes de descargar,  
   **Then** el sistema permite opcionalmente descargar la cartera y exclusión general consolidada o segmentada por cuenta según la necesidad operativa del encargo.

4. **Scenario 3.4 (Exportación cuando no existen candidatos en cartera o exclusión en el filtro):**  
   **Given** que para una cuenta cliente específica no existen candidatos en estado de cartera o exclusión activa,  
   **When** se solicita la descarga del reporte,  
   **Then** el sistema genera un archivo válido con las 5 cabeceras oficiales y una fila informativa limpia indicando "Sin registros en periodo de exclusión activa para la cuenta seleccionada", evitando descargas vacías o fallos de archivo corrupto.
...
Como Senior Technical Recruiter durante la llamada telefónica o Account Recruitment Coordinator al recibir una pretensión salarial,  
...
1. **Scenario 4.1 (Cálculo exacto del Costo Empresa mediante Factor legal 1.56 con Trazabilidad Financiera):**  
   **Given** que un candidato comunica una pretensión salarial bruta de S/. 5,000 mensuales en moneda nacional (PEN),  
   **When** la Senior Technical Recruiter autenticada registra dicho valor en la ficha de postulación seleccionando la modalidad "Bruto",  
   **Then** el sistema aplica de inmediato el multiplicador laboral de 1.56 derivado de las cargas sociales del Régimen Privado D.L. 728 (Gratificaciones legales, Bonificación extraordinaria 9%, CTS, EsSalud 9%, Vacaciones, Seguro Vida Ley), muestra el Costo Empresa (CTC Solicitado) exacto de S/. 7,800.00 y registra en la bitácora de auditoría la modificación salarial con autor, fecha/hora y valor previo y nuevo.
...
4. **Scenario 4.4 (Semáforo de viabilidad financiera):**  
   ...
   **Then** la Senior Technical Recruiter dispone de argumentos objetivos para negociar durante la llamada o tramitar excepciones con el Delivery Manager.

5. **Scenario 4.5 (Validación de límites y montos atípicos):**  
   **Given** que la Senior Technical Recruiter ingresa una expectativa salarial mensual menor al Sueldo Mínimo Vital (S/. 1,025) o superior a S/. 35,000 mensuales,  
...
Como Senior Technical Recruiter o Account Recruitment Coordinator,  
...
1. **Scenario 5.1 (Detección positiva de talento Boomerang elegible por DNI):**  
   **Given** que una Senior Technical Recruiter ingresa el DNI de un profesional que trabajó en TCS entre 2022 y 2024 y cuya salida fue voluntaria con evaluación favorable,  
...
2. **Scenario 5.2 (Alerta preventiva de candidato no recontratable):**  
   ...
   **Then** emite una advertencia de compliance de alta prioridad para la Senior Technical Recruiter, sugiriendo validar el caso con la gerencia de Recursos Humanos antes de coordinar cualquier entrevista con el cliente.
...
Como Senior Technical Recruiter, Account Recruitment Coordinator, Compliance Officer o Head of Talent Acquisition,  
...
**Independent Test**: Puede ser probado de forma independiente registrando una cuenta con correo institucional (`@tcs.com`) y credencial de acceso robusta, iniciando sesión para obtener acceso autorizado, navegando por el sistema bajo perfiles con diferentes roles corporativos (Senior Technical Recruiter, Head of Talent Acquisition, Account Recruitment Coordinator, Compliance Officer), ejecutando mutaciones de prueba (editar un campo de candidato, subir un CV en PDF, cambiar el estado del embudo, cargar una planilla de prueba y generar el reporte de cartera y exclusiones), verificando que la bitácora inmutable asiente de inmediato cada evento con el identificador del usuario actor, fecha/hora exacta con zona horaria, acción realizada y valores antes/después, e intentando realizar mutaciones con un rol no autorizado (Compliance Officer) o con una sesión cerrada/expirada comprobando el bloqueo preventivo y registro del intento.

**Acceptance Scenarios**:

1. **Scenario 6.1 (Registro e inicio de sesión seguro con rol inicial de mínimo privilegio):**  
   **Given** un integrante del equipo de selección (Head of Talent Acquisition, Senior Technical Recruiter, Account Recruitment Coordinator o Compliance Officer) que cuenta con correo institucional de la compañía (`@tcs.com`),  
   ...
   **Then** el sistema crea la cuenta de usuario de forma segura salvaguardando la confidencialidad de las credenciales sin exponerlas en texto claro, asignándole por defecto el rol de mínimo privilegio (`Compliance Officer`), y al iniciar sesión emite una sesión autenticada activa con expiración automática por inactividad, permitiendo únicamente navegación en modo consulta y auditoría hasta que un Head of Talent Acquisition formalice su elevación de permisos.

2. **Scenario 6.2 (Control de acceso basado en roles y delegación administrativa auditada de permisos):**  
   **Given** tres usuarios con diferentes perfiles operativos: Senior Technical Recruiter (operación ordinaria del embudo), Head of Talent Acquisition (gestión de usuarios, autorizaciones presupuestales y auditoría global) y Compliance Officer (modo consulta, auditoría y analítica),  
   **When** el usuario con rol de Compliance Officer intenta editar una ficha, subir un currículo o alterar el estado de una postulación,  
   **Then** el sistema bloquea de inmediato la operación de escritura, despliega un aviso formal de permiso denegado ("Acceso denegado: su rol solo posee permisos de consulta y auditoría"), preserva los datos intactos y registra en la bitácora de seguridad el intento de mutación no autorizada.  
   **And** cuando un Head of Talent Acquisition actualiza el rol de un usuario (ej. de `Compliance Officer` a `Senior Technical Recruiter`), el sistema valida la autoridad administrativa, actualiza el perfil y genera un asiento mandatorio en la bitácora de auditoría vinculando al Head of Talent Acquisition que autorizó el cambio, el usuario afectado, el rol previo, el nuevo rol asignado y la marca temporal precisa.

3. **Scenario 6.3 (Trazabilidad obligatoria y atómica en mutaciones de Ficha de Candidato y Postulación):**  
   **Given** una Senior Technical Recruiter autenticada con usuario `carla.soto@tcs.com` que visualiza la postulación activa de un candidato,  
...
5. **Scenario 6.5 (Trazabilidad de exportaciones regulatorias bajo Ley 29733 y simulaciones financieras):**  
   **Given** un usuario autenticado que solicita la generación y descarga del Reporte de Cartera y Exclusiones para Adecco o formaliza el cálculo de simulación CTC para una oferta,  
...
6. **Scenario 6.6 (Cierre de sesión seguro e invalidación inmediata de sesión activa):**  
   **Given** una Senior Technical Recruiter con una sesión autenticada activa en el sistema,  
...
7. **Scenario 6.7 (Consola Central de Auditoría Global y Exportación para Cumplimiento Ley 29733):**  
   **Given** un usuario autenticado con rol de Head of Talent Acquisition o Compliance Officer,  
   **When** accede a la consola central de auditoría del sistema y aplica filtros por rango de fechas, usuario ejecutor, entidad objetivo (Candidato, Postulación, Screening, Documento, Planilla_Adecco, Reporte_Cartera_Exclusiones, Usuario) o tipo de operación,  
   **Then** el sistema presenta el listado cronológico consolidado de eventos con sus valores previos y resultantes, y permite descargar un reporte estructurado de auditoría para fines de control interno o requerimientos formales bajo la Ley N° 29733, registrando en la propia bitácora la consulta y exportación del historial efectuada por el Head of Talent Acquisition o Compliance Officer.
...
2. **Falla o Indisponibilidad Temporal del Servicio de Identidad Nacional o Trabajo Offline:**  
   ¿Qué sucede si el servicio externo de consulta de DNI no responde, excede el tiempo límite de espera (timeout > 3 segundos) o se opera en ausencia de conectividad de red?  
   *Comportamiento del sistema*: El sistema primero resuelve inmediatamente por el almacén/caché local de identidades consultadas o registradas previamente, permitiendo el autollenado instantáneo sin depender de internet ni de cuotas externas. En ausencia de red para un DNI nuevo no registrado previamente, se degrada elegantemente (*graceful degradation*), despliega una advertencia ("Sin conexión al servicio de identidad; proceda con captura asistida"), permite a la Senior Technical Recruiter tipear los nombres y apellidos manualmente, y deja la marca de auditoría "Pendiente de regularización de identidad" con encolamiento para sincronización automática al restablecerse el servicio.

3. **Agotamiento de Cuota o Límite de Frecuencia del Servicio de Identidad Nacional (Límite por Segundo / Cuota Mensual):**  
   ...
   *Comportamiento del sistema*: El sistema captura el estado, notifica al Head of Talent Acquisition mediante alerta interna ("Límite de consultas de identidad alcanzado"), conmuta de forma automática y transparente a modo de ingreso manual asistido para las Senior Technical Recruiters, y no interrumpe el flujo de registro de candidatos.
...
4. **DNI de 8 Dígitos Inexistente o Cancelado en Padrón Nacional:**  
   ...
   *Comportamiento del sistema*: El sistema informa el motivo exacto devuelto por el servicio oficial ("DNI no hallado en padrón nacional" o "Documento no vigente"), bloquea la continuidad automática y exige a la Senior Technical Recruiter confirmar el número con el candidato antes de proceder con registro manual observado.
...
8. **Simulación Financiera con Presupuesto Cero, Negativo o Vacío:**  
   ¿Cómo se comporta la calculadora de variación presupuestal si la Senior Technical Recruiter ingresa una pretensión salarial pero el presupuesto del rol está en blanco, en cero (`0.00`) o con valor negativo por error?  
...
9. **CVs Ilegibles, Protegidos con Contraseña o Escaneados sin Texto Seleccionable:**  
   ¿Qué ocurre cuando la Senior Technical Recruiter sube un CV en PDF que es una imagen escaneada borrosa o tiene clave de apertura?  
   *Comportamiento del sistema*: El sistema detecta el error de lectura, informa a la Senior Technical Recruiter que el documento no posee texto procesable y habilita la edición manual de la ficha, almacenando el archivo adjunto para consulta visual humana sin interrumpir el flujo.
...
10. **Candidato con Múltiples Postulaciones Simultáneas a Diferentes Cuentas:**  
    ...
    *Comportamiento del sistema*: La arquitectura desacopla la ficha de identidad (`Candidato`) de sus participaciones en procesos (`Postulacion`). El candidato conserva una única ficha maestra y puede tener múltiples postulaciones asociadas, emitiendo una alerta a la Senior Technical Recruiter si se detecta que ya tiene un proceso activo en curso para evitar duplicidad de entrevistas y definir ownership entre Senior Technical Recruiters.

11. **Exportación de Cartera y Exclusiones para Adecco sin Registros Elegibles:**  
    ¿Qué sucede si se solicita la descarga del reporte de cartera y exclusiones y no existen candidatos en estado de cartera o exclusión activa para los filtros aplicados?  
    *Comportamiento del sistema*: El sistema genera el archivo con las 5 columnas oficiales y un aviso informativo indicando que no hay registros excluidos para los parámetros seleccionados, impidiendo descargas fallidas o archivos corruptos.
...
13. **Intentos Reiterados de Autenticación Fallida y Bloqueo Preventivo de Cuenta:**  
    ...
    *Comportamiento del sistema*: El sistema incrementa el contador de intentos fallidos, responde con un mensaje neutral que no revele la existencia previa del correo ("Credenciales no válidas"), bloquea preventivamente el acceso a la cuenta durante 15 minutos al superar los 5 intentos y genera un evento de auditoría de seguridad detallando el identificador ingresado y la marca de tiempo. La cuenta se desbloquea de manera automática al culminar dicho lapso o de forma anticipada mediante intervención manual autorizada de un Head of Talent Acquisition.

14. **Expiración de Sesión por Inactividad durante Edición de Ficha o Carga de Datos:**  
    ¿Qué sucede si una Senior Technical Recruiter deja una ficha de candidato o formulario de llamada abierto sin interactuar durante un lapso prolongado (ej. 30 minutos) y luego presiona "Guardar"?  
    *Comportamiento del sistema*: El sistema detecta la sesión expirada, retiene localmente en el navegador los datos modificados para proteger el trabajo de la Senior Technical Recruiter, despliega una ventana modal de reautenticación solicitando credenciales, y tras una validación exitosa, aplica la mutación registrando en la bitácora de auditoría al usuario autenticado sin pérdida de información.

15. **Solicitud de Mutación sin Autenticación Activa o con Permisos Insuficientes:**  
    ¿Qué sucede si se envía una solicitud de modificación de datos, subida de documentos o exportación sin una sesión autenticada válida o desde una cuenta con permisos restringidos (ej. rol Compliance Officer intentando editar un candidato)?  
...
16. **Garantía de Inmutabilidad y Bloqueo de Alteración Retroactiva en la Bitácora de Auditoría:**  
    ¿Puede un usuario con privilegios de Head of Talent Acquisition o Account Recruitment Coordinator editar, sobrescribir o eliminar registros de la bitácora de auditoría para encubrir un error o modificación indebida?  
...
17. **Edición Concurrente Simultánea sobre la Misma Ficha o Postulación (Control de Concurrencia Optimista):**  
    ¿Qué sucede si dos Senior Technical Recruiters abren simultáneamente la misma ficha de candidato o postulación y ambas intentan guardar modificaciones distintas (ej. una actualiza la expectativa salarial y la otra cambia el estado del embudo)?  
    *Comportamiento del sistema*: El sistema implementa control de concurrencia optimista basado en la versión del registro. Al intentar guardar la segunda Senior Technical Recruiter, el sistema detecta que los datos base ya fueron modificados por otra usuaria, bloquea la sobreescritura ciega, despliega una alerta con la vista comparativa de diferencias ("El registro fue modificado por [Senior Technical Recruiter A] a las [Hora]. Revise las diferencias antes de guardar") y permite a la usuaria revalidar o conciliar sus cambios, registrando eventos de auditoría independientes para cada una de las mutaciones aplicadas.

18. **Interrupción de Conexión de Red o Expiración de Sesión durante Carga Masiva de Planillas de Adecco o Subida de CVs:**  
    ¿Qué ocurre si la conexión de red se interrumpe o la sesión expira en medio de la transferencia o procesamiento de una planilla de 100 postulantes o un archivo de currículo?  
    *Comportamiento del sistema*: Las operaciones de ingesta documental y procesamiento masivo se rigen bajo estricta atomicidad transaccional (todo o nada). Si el proceso se corta antes de su confirmación final, el sistema revierte automáticamente cualquier inserción o cambio en progreso sin dejar registros huérfanos ni candidatos a medio procesar, notifica al usuario del error de transferencia con opción de reintento, y consigna en la bitácora de auditoría un evento de carga fallida/interrumpida para fines de diagnóstico y soporte.

19. **Desbloqueo Manual Administrativo de Cuentas Bloqueadas por Intentos Fallidos:**  
    ¿Puede un Head of Talent Acquisition desbloquear la cuenta de una Senior Technical Recruiter que superó los 5 intentos fallidos antes de que transcurran los 15 minutos de penalización automática?  
    *Comportamiento del sistema*: El sistema faculta a los usuarios con rol de Head of Talent Acquisition a ejecutar un desbloqueo manual inmediato previa confirmación de identidad de la colaboradora. Al efectuarse el desbloqueo, el sistema restablece el contador de intentos fallidos, reactiva la cuenta al instante y asienta de forma obligatoria en la bitácora de auditoría el identificador del Head of Talent Acquisition autorizador, el usuario desbloqueado, la justificación operativa y la marca temporal exacta.
...
- **FR-002**: Al ingresar un número de DNI peruano de 8 dígitos, el sistema DEBE resolver la consulta en primer orden consultando inmediatamente el almacén/caché local de identidades previamente consultadas o registradas (permitiendo autollenado instantáneo sin depender de conexión a internet ni de cuotas externas); si el documento no reside en la caché local, DEBE consultar en tiempo real el servicio de validación de identidad nacional y autocompletar de forma obligatoria: nombres de pila, apellido paterno, apellido materno, fecha de nacimiento y distrito de residencia, persistiendo los datos obtenidos en la caché local para futuras operaciones offline.
- **FR-003**: En caso de ausencia de conectividad de red o indisponibilidad del servicio externo para un DNI nuevo no presente en la caché local, así como ante agotamiento de cuotas del proveedor o documento no hallado en el padrón, el sistema DEBE degradarse elegantemente admitiendo la captura asistida mediante digitación manual, asignando la marca de auditoría "Pendiente de regularización de identidad" y encolando la solicitud para sincronización y validación automática al restablecerse el servicio.
...
- **FR-008**: Si se detecta una coincidencia por documento, teléfono o correo, el sistema DEBE bloquear la creación de un nuevo registro huérfano y redirigir a la Senior Technical Recruiter hacia la ficha existente con su historial consolidado.
...
- **FR-010**: Al cargarse un currículo, el sistema DEBE extraer de manera estructurada y asistida las herramientas tecnológicas, años de experiencia, roles previos e idiomas, poniéndolos a disposición de la Senior Technical Recruiter para su validación sin alterar la autoridad humana sobre el dato.
...
#### Generador de Reporte de Cartera y Exclusiones a Demanda bajo Ley 29733 (Proceso 3 y Gobernanza de Proveedores)
- **FR-020**: El sistema DEBE incluir un generador a demanda del Reporte de Cartera y Exclusiones para Adecco, ejecutable con un solo clic en cualquier instante sin restricción de día u horario.
- **FR-021**: El Reporte de Cartera y Exclusiones DEBE estructurar de manera uniforme exactamente las 5 columnas estructurales oficiales por registro: `DNI`, `Nombres y Apellidos`, `Perfil`, `Vigencia de Exclusión` y `Estado`.
- **FR-022**: En cumplimiento estricto de la Ley N° 29733 (Protección de Datos Personales), el sistema DEBE omitir de forma absoluta del reporte de cartera y exclusiones cualquier dato de pretensión salarial, tarifas presupuestales, teléfonos personales, correos, notas privadas de evaluación y motivos sensibles de descarte.
- **FR-023**: El sistema DEBE registrar en una bitácora de auditoría inmutable cada generación de reporte de cartera y exclusiones, detallando usuario que lo solicitó, marca de tiempo y cantidad de registros exportados.
...
- **FR-028**: El sistema DEBE proveer un semáforo visual financiero que alerte a la Senior Technical Recruiter durante la llamada telefónica si la pretensión del candidato se encuentra dentro del presupuesto (verde), con desvío moderado de hasta 10% (ámbar) o desvío crítico superior al 10% (rojo).
...
- **FR-032**: Si el registro histórico señala al ex-colaborador como no elegible para reingreso (*Do Not Rehire*), el sistema DEBE emitir una advertencia de compliance para que la Senior Technical Recruiter detenga el avance del proceso antes de comprometer entrevistas con el cliente.

#### Principios Constitucionales de Gobernanza y Human-in-the-Loop
- **FR-033**: El sistema DEBE garantizar que ninguna funcionalidad de asistencia o inteligencia artificial descarte automáticamente a un candidato, modifique unilateralmente su estado o tome decisiones de contratación; toda transición de estado y descarte DEBE ser ejecutada y ratificada por una persona humana (Senior Technical Recruiter o Head of Talent Acquisition).
- **FR-034**: La llamada telefónica de validación técnica y screening (con sus 7 dimensiones de evaluación) DEBE ser realizada íntegramente por un ser humano (Senior Technical Recruiter), limitándose el sistema a brindar fichas, alertas de conmutación y resúmenes de soporte.
...
- **FR-037**: El sistema DEBE proveer un mecanismo seguro de registro de cuentas para usuarios corporativos, requiriendo nombres y apellidos completos, correo electrónico institucional (`@tcs.com`) y credencial de acceso que satisfaga políticas de longitud y robustez. Las cuentas recién registradas DEBEN crearse con el rol de mínimo privilegio (`Compliance Officer`) por defecto, requiriendo que un usuario con rol `Head of Talent Acquisition` asigne o eleve formalmente los permisos a `Senior Technical Recruiter`, `Account Recruitment Coordinator` o `Head of Talent Acquisition` para habilitar operaciones de escritura en el pipeline.
...
- **FR-039**: El sistema DEBE implementar un modelo de Control de Acceso Basado en Roles (RBAC) con la nomenclatura y niveles de permisos corporativos internacionales:
  - *Head of Talent Acquisition* (Admin/Gobernanza): Todos los privilegios operativos, administración y asignación de roles de usuario, autorizaciones presupuestales salariales fuera de banda y acceso integral a la bitácora global de auditoría.
  - *Senior Technical Recruiter* (Operador principal): Creación y modificación de fichas de candidatos, gestión del embudo de postulaciones asignadas, registro estructurado del screening telefónico, carga y extracción de CVs, validación masiva de planillas de Adecco y generación del reporte de cartera y exclusiones.
  - *Account Recruitment Coordinator*: Coordinación de vacantes por cuenta cliente, asignación y reasignación de postulaciones a Senior Technical Recruiters, supervisión operativa del embudo y consulta de métricas de entrega.
  - *Compliance Officer* (Auditor/Observador): Acceso en modo de solo lectura y auditoría a fichas, postulaciones, consola central de auditoría global y reportes de cumplimiento normativo bajo Ley N° 29733, con prohibición estricta de crear registros, mutar datos o cargar archivos.
...
- **FR-041**: El sistema DEBE asociar de manera obligatoria, automática e indisociable al usuario autenticado en sesión con cada una de las siguientes operaciones del pipeline:
  ...
  - Reasignación de postulación o transferencia de Senior Technical Recruiter asignado a una vacante.
  ...
  - Generación y descarga a demanda de Reportes de Cartera y Exclusiones bajo Ley N° 29733.
  - Registro estructurado de las 7 dimensiones y dictamen cualitativo de la llamada humana de screening telefónico.
  - Registro de nuevos usuarios, elevación o modificación de roles operativos, y alteración del estado de cuentas (bloqueo, suspensión o desbloqueo manual).
  - Actualización o reseteo de credenciales confidenciales de acceso.
- **FR-042**: El sistema DEBE persistir cada evento de mutación o acción relevante en una bitácora inmutable de auditoría (*append-only*), registrando obligatoriamente: identificador único del evento, identificador y correo del usuario ejecutor, rol del usuario al momento de la acción, marca de tiempo precisa con zona horaria, tipo de entidad afectada (Candidato, Postulación, Screening, Documento, Planilla, Reporte_Cartera_Exclusiones, Usuario), identificador del registro afectado, versión del registro para soporte de concurrencia, tipo de operación (Creación, Modificación, Carga de Archivo, Exportación, Transición de Estado, Autenticación, Acceso Denegado, Modificación de Rol, Desbloqueo Manual, Fallo de Carga), en modificaciones de datos el detalle comparativo de los valores previos y nuevos valores resultantes, y justificación operativa cuando aplique a descartes o aprobaciones excepcionales.
...
- **FR-045**: El sistema DEBE registrar en la bitácora de seguridad los eventos de control de acceso, tales como inicios de sesión exitosos, intentos fallidos de autenticación, bloqueos temporales por reiteración de fallos, desbloqueos manuales por el Head of Talent Acquisition e intentos de ejecución de operaciones no autorizadas por rol.
- **FR-046**: El sistema DEBE proveer a los usuarios con rol de *Head of Talent Acquisition* y *Compliance Officer* una consola central de auditoría global con capacidades de consulta y filtrado avanzado (por rango de fechas/horas, usuario ejecutor, entidad objetivo, tipo de acción e identificador de registro), permitiendo auditar la totalidad de las mutaciones operativas y eventos de seguridad, inspeccionar los diferenciales de valores antes/después y exportar reportes consolidados en formato estructurado para revisiones de cumplimiento de la Ley N° 29733 y control interno.
- **FR-047**: El sistema DEBE incorporar control de concurrencia optimista en todas las entidades sujetas a edición (Candidatos y Postulaciones) mediante control de versión de registro; si dos usuarios intentan modificar concurrentemente el mismo registro, el sistema DEBE detectar la colisión de versión, impedir la sobreescritura ciega de los datos, alertar al segundo usuario con el detalle comparativo de las modificaciones aplicadas previamente y requerir su validación o conciliación antes de persistir, auditando ambos eventos de forma independiente.
- **FR-048**: El sistema DEBE asegurar la atomicidad transaccional en las operaciones de ingesta masiva de planillas de Adecco y subida de archivos de currículo; ante cualquier fallo de red, desconexión del cliente o expiración sobrevenida de la sesión durante la carga, la operación DEBE revertirse completamente en su totalidad (*rollback* atómico), impidiendo la persistencia de registros parciales, huérfanos o corrompidos, y asentando el evento fallido en la bitácora de auditoría.
...
- **Candidato (Ficha de Identidad Centralizada):**  
  Representa a la persona física individual como entidad única en la organización. Sus atributos funcionales clave comprenden: Tipo y Número de Documento de Identidad (DNI/CE/Pasaporte, único), Nombres de Pila, Apellido Paterno, Apellido Materno, Nombres Completos Normalizados (sin partículas), Teléfono Móvil Canónico (estándar E.164, único), Correo Electrónico Principal (único), Fecha de Nacimiento, Edad Calculada Dinámicamente, Ubigeo / Distrito de Residencia, Condición de Ex-Colaborador TCS (Boomerang), Estado de Validación de Identidad (`Validado Oficialmente`, `Pendiente de Regularización de Identidad`), y Marcas de Auditoría Cronológica. Posee una relación 1:N con las Postulaciones a procesos.

- **Postulación a Proceso (Ciclo de Vida en Embudo):**  
  Modela la participación formal de un candidato en una vacante o requerimiento específico. Sus atributos funcionales comprenden: Identificador de Postulación, Referencia al Candidato, Cuenta o Cliente Corporativo (ej. BCP, Entel, Banco Falabella), Identificador de Requerimiento (RGS/Vacante), Perfil Técnico Solicitado, Senior Technical Recruiter Responsable Asignado, Fuente de Reclutamiento de Origen (Adecco, Offshore, BYB, LinkedIn), Trimestre Fiscal Corporativo (Q1, Q2, Q3, Q4), Estado Operativo del Embudo (`Nuevo`, `En proceso`, `Screening telefónico`, `Pendiente entrevistas`, `Pendiente envío cliente`, `Oferta aceptada`, `No apto`, `Desistió`), Motivo Detallado de Cierre/Descarte (con clasificación entre `Excluyente Permanente` o `Temporal No Excluyente`), Disponibilidad de Incorporación, Observaciones Cualitativas y Trazabilidad Cronológica.

- **Screening Telefónico y Validación Cualitativa (Registro Humano):**  
  Entidad vinculada a la postulación que captura los resultados objetivos y cualitativos de la llamada telefónica humana. Atributos: Identificador de Screening, Referencia a la Postulación, Senior Technical Recruiter Evaluador, Fecha y Hora de la Llamada, Disponibilidad de Incorporación (`Inmediata`, `1 semana`, `2 semanas`, `1 mes`), Modalidad Aceptada (`Híbrido`, `Remoto`, `Presencial`), Viabilidad de Traslado Geográfico (Distrito Residencia vs Sede Cliente con indicador de conmutación), Resumen Técnico Validado, Notas Cualitativas de la Entrevista, y Dictamen Humano de Avance (`Avanza a Entrevista Técnica`, `No Apto`, `Enfriar/Recontactar`).
...
- **Lote de Planilla de Proveedor (Ingesta Adecco):**  
  Entidad que registra cada sesión de carga masiva de candidatos remitidos por agencias externas. Atributos: Identificador del Lote, Nombre del Proveedor, Fecha y Hora de Carga, Senior Technical Recruiter que Ingesta, Nombre del Archivo Original, Total de Filas Leídas, Cantidad de Duplicados Rojos, Cantidad de Reactivables Amarillos, Cantidad de Perfiles Limpios Verdes y Cantidad de Ex-Colaboradores TCS Detectados.

- **Reporte de Cartera y Exclusiones de Proveedor (Exportación a Demanda):**  
  Entidad que audita las descargas del reporte de cartera y exclusiones para proveedores bajo la Ley N° 29733. Consolida de manera uniforme las 5 columnas estructurales oficiales: `DNI`, `Nombres y Apellidos`, `Perfil`, `Vigencia de Exclusión` y `Estado`. Atributos funcionales y de auditoría: Identificador de Reporte, Destinatario (Adecco), Fecha y Hora de Generación, Usuario Solicitante, Cuenta/Cliente Filtrado (o consolidado general), Cantidad de Candidatos en Cartera/Excluidos, y Periodo de Vigencia de la Exclusión.

- **Usuario (Operador del Sistema y Cuenta Corporativa):**  
  Representa a la persona física individual autorizada para operar el sistema en sus distintas capacidades. Sus atributos funcionales comprenden: Identificador de Usuario, Nombres y Apellidos Completos, Correo Electrónico Corporativo Oficial (único, dominio institucional `@tcs.com`), Cargo Corporativo / Rol Asignado (`Head_of_Talent_Acquisition`, `Senior_Technical_Recruiter`, `Account_Recruitment_Coordinator`, `Compliance_Officer`), Estado de la Cuenta (`Activa`, `Suspendida`, `Bloqueada por Intentos Fallidos`), Credencial de Acceso Confidencial, Contador de Intentos Fallidos de Autenticación, Fecha y Hora del Último Acceso Exitoso, Marcas Temporales de Creación y Modificación, y Usuario Head of Talent Acquisition que Autorizó/Modificó el Rol. Posee relaciones 1:N con los Candidatos creados, las Postulaciones asignadas, los Lotes de Planilla de Proveedor ingestados, los Reportes de Cartera y Exclusiones solicitados y los Registros de Auditoría emitidos.

- **Registro de Auditoría / Bitácora de Cambios (Trazabilidad Inmutable):**  
  Modela el asiento cronológico e inmutable de toda acción, mutación de datos, subida de documentos, exportación regulatoria o evento de seguridad ejecutado en el sistema. Atributos funcionales: Identificador Único de Evento, Referencia al Usuario Actor (Identificador y Correo Institucional), Rol Operativo del Usuario al momento de la operación, Marca de Tiempo Precisa (fecha, hora exacta y zona horaria), Tipo de Acción Realizada (`Creación`, `Modificación`, `Carga_Archivo`, `Exportación`, `Transición_Estado`, `Autenticación`, `Acceso_Denegado`, `Modificación_Rol`, `Desbloqueo_Manual`, `Fallo_Carga`), Entidad Objeto Afectada (`Candidato`, `Postulación`, `Screening`, `Documento`, `Planilla_Adecco`, `Reporte_Cartera_Exclusiones`, `Usuario`), Identificador del Registro Objeto Afectado, Versión del Registro (para control de concurrencia optimista), Detalle Estructurado de la Mutación (nombre del campo modificado, valor previo y nuevo valor resultante), Motivo o Justificación Operativa (mandatorio en descartes y excepciones presupuestales), y Metadatos de Integridad Documental (nombre original del archivo, tipo de documento, tamaño en bytes e identificador de integridad para archivos adjuntos).
...
- **SC-005 (Protección Legal y Anonimización Estricta Ley 29733):**  
  Garantizar un 100% de cumplimiento normativo en los reportes de cartera y exclusiones generados para proveedores externos, con 0.00% de filtración de remuneraciones, tarifas de facturación, teléfonos personales, correos, notas privadas de evaluación o atributos demográficos protegidos.
- **SC-006 (Incremento de Efectividad del Proveedor Externo Adecco):**  
  Elevar el ratio de perfiles útiles remitidos por Adecco del 10% actual (donde 9 de cada 10 son repetidos o inadecuados) a al menos 35% en los primeros 60 días de entrega regular del reporte de cartera y exclusiones a demanda.
...
- **SC-008 (Trazabilidad y Responsabilidad Operativa al 100%):**  
  Garantizar que el 100% de las mutaciones de datos, cambios de estado en el embudo, subida de CVs, ingesta de planillas masivas, exportaciones de reportes de cartera y exclusiones y modificaciones de roles queden vinculadas a un usuario autenticado con marca de tiempo precisa en la bitácora inmutable, con 0.00% de operaciones anónimas permitidas o huérfanas de autoría.
- **SC-009 (Seguridad de Acceso y Control de Roles):**  
  Lograr una tasa de 100% de efectividad en el bloqueo de accesos no autorizados a funciones restringidas (ej. edición o carga denegada para rol Compliance Officer) y una tasa de 0.00% de exposición de credenciales en texto claro o de sesiones activas sin invalidar tras el cierre de sesión.
- **SC-010 (Integridad de Datos ante Concurrencia y Fallas de Red):**  
  Lograr una tasa de 0.00% de sobreescrituras ciegas de datos en modificaciones concurrentes mediante el control de concurrencia optimista, y 0.00% de registros huérfanos o corrompidos ante interrupciones de red o sesiones expiradas durante la ingesta masiva de archivos.

---

## Assumptions

- **A-001 (Soporte Offline, Caché Local y Disponibilidad de Identidad):** Se asume que ante una consulta de DNI el sistema resuelve prioritariamente a través del almacén/caché local de identidades previamente consultadas o registradas, posibilitando el autollenado inmediato sin dependencia de conexión a internet ni consumo de cuotas externas; ante caídas temporales de red, demoras (>3s), agotamiento de cuotas o ausencia de conectividad para DNIs inéditos, se asume la continuidad operativa mediante captura asistida con la marca de auditoría "Pendiente de regularización de identidad" y encolamiento para sincronización automática asíncrona al restablecerse el servicio.
...
- **A-004 (Autoridad y Juicio Humano Innegociable):** Se asume que el Senior Technical Recruiter siempre tiene la facultad soberana de revisar, editar, confirmar o rectificar cualquier sugerencia, extracción o advertencia generada por el sistema antes de guardar o derivar un perfil a una cuenta cliente.
...
- **A-006 (Hardware, Conectividad y Entorno del Usuario):** Se asume que los integrantes del equipo de Selección (Senior Technical Recruiters, Account Recruitment Coordinators, Compliance Officers y Head of Talent Acquisition) operan desde estaciones de trabajo corporativas de TCS con navegadores web modernos y conectividad a la red interna para interactuar con WhatsApp Web y los servicios de selección.
- **A-007 (Acuerdo Operativo con Adecco):** Se asume que el proveedor externo Adecco aceptará recibir y utilizar el reporte de cartera y exclusiones a demanda como filtro previo mandatario antes de iniciar sus jornadas de búsqueda y remitir sus planillas semanales.
...
- **A-010 (Gestión de Identidades y Roles Corporativos Internacionales):** Se asume que todos los colaboradores autorizados del equipo de Selección cuentan con un correo corporativo oficial institucional de TCS (`@tcs.com`), que las cuentas registradas se inicializan bajo el principio de mínimo privilegio en el rol `Compliance Officer` (modo observador/auditor), y que la administración y elevación de roles a `Senior Technical Recruiter`, `Account Recruitment Coordinator` o `Head of Talent Acquisition` es gestionada exclusivamente por los usuarios con rol `Head of Talent Acquisition`, garantizando la trazabilidad integral de sus operaciones.
