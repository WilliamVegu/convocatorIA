...
1. **Scenario 1.1 (Autollenado exitoso por DNI y registro auditable de creación):**  
   **Given** que la reclutadora se encuentra en el formulario de creación de ficha de candidato con una sesión autenticada activa,  
   **When** ingresa un número de DNI peruano válido de 8 dígitos (ej. `76128709`) y presiona validar o cambia de campo,  
   **Then** el sistema consulta el servicio de validación de identidad nacional, autocompleta de inmediato los campos de nombres de pila, apellido paterno, apellido materno, fecha de nacimiento y distrito de residencia, calculando la edad cronológica exacta a la fecha actual, bloqueando la edición de campos de identidad oficial para evitar alteraciones accidentales, y consignando al guardar la autoría del registro a la reclutadora autenticada en la bitácora inmutable de auditoría.
...
4. **Scenario 1.4 (Extracción estructurada inicial de CV adjunto con custodia auditable):**  
   **Given** que la reclutadora autenticada carga un archivo de currículo en formato estándar (PDF o Word) al crear o editar una ficha,  
...
5. **Scenario 1.5 (Captura de las 7 dimensiones de la llamada humana con autoría individual y alerta geográfica):**  
   **Given** que la reclutadora autenticada realiza la llamada telefónica de screening al candidato,  
...
4. **Scenario 2.4 (Clasificación 🟢 Verde - Candidato Inédito y Limpio con Trazabilidad de Ingesta):**  
   ...
   **When** concluye la evaluación del archivo y la reclutadora autenticada confirma la importación,  
...
1. **Scenario 4.1 (Cálculo exacto del Costo Empresa mediante Factor legal 1.56 con Trazabilidad Financiera):**  
   ...
   **When** la reclutadora autenticada registra dicho valor en la ficha de postulación seleccionando la modalidad "Bruto",  
...
1. **Scenario 6.1 (Registro e inicio de sesión seguro con rol inicial de mínimo privilegio):**  
   ...
   **Then** el sistema crea la cuenta de usuario de forma segura salvaguardando la confidencialidad de las credenciales sin exponerlas en texto claro, asignándole por defecto el rol de mínimo privilegio (`Observador`), y al iniciar sesión emite una sesión autenticada activa con expiración automática por inactividad, permitiendo únicamente navegación en modo consulta hasta que un administrador formalice su elevación de permisos.
...
2. **Scenario 6.2 (Control de acceso basado en roles y delegación administrativa auditada de permisos):**  
   ...
   **Then** el sistema bloquea de inmediato la operación de escritura, despliega un aviso formal de permiso denegado ("Acceso denegado: su rol solo posee permisos de consulta"), preserva los datos intactos y registra en la bitácora de seguridad el intento de mutación no autorizada.  
   **And** cuando un Administrador/Coordinador actualiza el rol de un usuario (ej. de `Observador` a `Reclutadora`), el sistema valida la autoridad administrativa, actualiza el perfil y genera un asiento mandatorio en la bitácora de auditoría vinculando al administrador que autorizó el cambio, el usuario afectado, el rol previo, el nuevo rol asignado y la marca temporal precisa.
...
7. **Scenario 6.7 (Consola Central de Auditoría Global y Exportación para Cumplimiento Ley 29733):**  
   **Given** un usuario autenticado con rol de Coordinador / Administrador,  
   **When** accede a la consola central de auditoría del sistema y aplica filtros por rango de fechas, usuario ejecutor, entidad objetivo (Candidato, Postulación, Documento, Planilla, Usuario) o tipo de operación,  
   **Then** el sistema presenta el listado cronológico consolidado de eventos con sus valores previos y resultantes, y permite descargar un reporte estructurado de auditoría para fines de control interno o requerimientos formales bajo la Ley N° 29733, registrando en la propia bitácora la consulta y exportación del historial efectuada por el administrador.
...
17. **Edición Concurrente Simultánea sobre la Misma Ficha o Postulación (Control de Concurrencia Optimista):**  
    ¿Qué sucede si dos reclutadoras abren simultáneamente la misma ficha de candidato o postulación y ambas intentan guardar modificaciones distintas (ej. una actualiza la expectativa salarial y la otra cambia el estado del embudo)?  
    *Comportamiento del sistema*: El sistema implementa control de concurrencia optimista basado en la versión del registro. Al intentar guardar la segunda reclutadora, el sistema detecta que los datos base ya fueron modificados por otra usuaria, bloquea la sobreescritura ciega, despliega una alerta con la vista comparativa de diferencias ("El registro fue modificado por [Reclutadora A] a las [Hora]. Revise las diferencias antes de guardar") y permite a la usuaria revalidar o conciliar sus cambios, registrando eventos de auditoría independientes para cada una de las mutaciones aplicadas.
...
19. **Desbloqueo Manual Administrativo de Cuentas Bloqueadas por Intentos Fallidos:**  
    ¿Puede un Coordinador o Administrador desbloquear la cuenta de una reclutadora que superó los 5 intentos fallidos antes de que transcurran los 15 minutos de penalización automática?  
    *Comportamiento del sistema*: El sistema faculta a los usuarios con rol de Coordinador/Administrador a ejecutar un desbloqueo manual inmediato previa confirmación de identidad de la colaboradora. Al efectuarse el desbloqueo, el sistema restablece el contador de intentos fallidos, reactiva la cuenta al instante y asienta de forma obligatoria en la bitácora de auditoría el identificador del administrador autorizador, el usuario desbloqueado, la justificación operativa y la marca temporal exacta.
...
- **FR-041**: El sistema DEBE asociar de manera obligatoria, automática e indisociable al usuario autenticado en sesión con cada una de las siguientes operaciones del pipeline:
  ...
  - Reasignación de postulación o transferencia de reclutadora responsable asignada a una vacante.
